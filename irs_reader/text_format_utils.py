        ...
        for result in results:
            for this_result in result["csv_line_array"]:
                ...
                try:
                    ...
                    pass
                if vardata:
                    this_result["variable_name"] = (
                        vardata["db_table"] + "." + vardata["db_name"]
                    )

                raw_line_num = vd.get_line_number(
                    this_result["xpath"], parsed_filing.get_version()
                )
...
def to_txt(
    parsed_filing, standardizer=None, documentation=True, vd=None, outfilepath=None
):
    if not vd:
        vd = VersionDocumentizer()
    results = parsed_filing.get_result()
