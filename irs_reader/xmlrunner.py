...
class XMLRunner(object):
    ...
    def run_filing(self, object_id, verbose=False):
        ...
        if this_version in ALLOWED_VERSIONSTRINGS or (
            self.csv_format and this_version in CSV_ALLOWED_VERSIONSTRINGS
        ):
            this_version = this_filing.get_version()
            ...
            if (
                verbose and not self.csv_format
            ):  # csv format works on years with many, many keyerrors,
