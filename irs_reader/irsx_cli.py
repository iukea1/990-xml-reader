...
def get_parser():
    ...
    parser.add_argument(
        "object_ids", metavar="object_ids", type=int, nargs="+", help="object ids"
    )
    parser.add_argument(
        "--verbose",
        dest="verbose",
    ...
    parser.add_argument("--file", default=None, help="Write result to file")
    parser.add_argument(
        "--list_schedules",
        dest="list_schedules",
        action="store_const",
        ...
        if args_read.list_schedules:
            ...
            if args_read.schedule:
                parsed_filing = xml_runner.run_sked(
                    object_id, args_read.schedule, verbose=args_read.verbose
                )
            else:
                parsed_filing = xml_runner.run_filing(
                    object_id, verbose=args_read.verbose
                )
